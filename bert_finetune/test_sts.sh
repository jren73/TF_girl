...
program="$program_home/BERT_STS-B/run_scorer.py \
  --task_name=STS \
  --do_train=true \
  --do_eval=true \
...
for memsize in $FASTMEM_SIZE_LIST; do
	...
	./launch_testee.sh      --thp-migration=0 \
                    	    ...
                    	    python $program "--output_dir=$program_home/BERT_STS-B/sts_output_$memsize" "--version=test_fastmem_$memsize""_MB_thp_0"
